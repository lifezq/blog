...
class LoginAuth{
    ...
    public static function checkUser($username, $password, $imgcode, $logincode = false) {
        ...
        session_start();
        // echo "<script>var login_n=window.top.document.getElementById('login-error'); login_n.style.display='none';</script>";
     ...
     * 注册页面
     */
    public static function registerPage($errorCode = NULL) {
        ...
        if ($errorCode) {
            switch ($errorCode) {
                ...
                case self::LOGIN_ERROR_PASSWD:
                    ...
                    break;
                case self::LOGIN_ERROR_EMAIL:
                    $error_msg = '您的帐号还未经过激活，请先激活后再登录，如有问题，请联系管理员';
                    break;
            }
        }
        
        //如果是从浮动登录窗口登录
        if(isset($_GET['op']) && $_GET['op']=='floated'){
            echo "<script> function _jswrite(obj,str){ if(obj.innerHTML){ obj.innerHTML=str;}else{obj.innerTEXT=str;}}</script>";
            echo "<script>var login_n=window.top.document.getElementById('login-error'); login_n.style.display='block'; _jswrite(login_n,'".$error_msg."');</script>";
            exit;
        }

        require_once View::getView('login_user');
        View::output();
    }
    ...
    private static function getUserDataByLogin($userLogin) {
        ...
        $userData = false;
        $userData = $DB->fetch_array($DB->query("SELECT * FROM ".DB_PREFIX."user WHERE username = '$userLogin'"));
        if (!$userData['uid']) {
            return false;
        }elseif(!$userData['emailcheck']){
            return -1;
        }
    ...
    public static function setAuthCookie($user_login, $ispersis = false) {
        ...
        setcookie($auth_cookie_name, $auth_cookie, $expiration,'/', WEB_COOKIE_DOMAIN);
    }
