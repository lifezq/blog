...
class Log_Model {
	...
	function __construct() {
        ...
        function is_user_com($gid){
            $uids=$this->db->once_fetch_array("select uids from ". DB_PREFIX ."com_uids where gid=$gid");
            $uids=$uids['uids'];
        ...
        /*
         * 搜索关键字  keyword
         * $gethot为我时为获取最热的三个搜索词
         */
        function keyword($keyword='',$gethot=0){
            if($gethot == 1){
                $sql="select keyname from ". DB_PREFIX ."keyword order by knum desc limit 3";
            ...
            $keyword = addslashes(htmlspecialchars(urldecode(trim($keyword))));
            $keyword = str_replace(array('%', '_'), array('\%', '\_'), $keyword);
            $sql="select kid from ". DB_PREFIX ."keyword where keyname='$keyword' ";
            $res = $this->db->query($sql);
	    $row = $this->db->fetch_array($res);
            if($row['kid']){
               $this->db->query("update ". DB_PREFIX ."keyword set knum=knum+1 where keyname='$keyword' "); 
            }else{
               $this->db->query("insert into ". DB_PREFIX ."keyword(keyname,knum) values('$keyword',1)");
            }
        }
		...
		while ($row = $this->db->fetch_array($res)) {
			...
			if (!empty($row['password']) && $cookiePassword != $row['password']) {
				...
				if (!empty($row['excerpt'])) {
					$row['excerpt'] = substring(strip_tags($row['excerpt']),0,300).'&nbsp;&nbsp;&nbsp;&nbsp;<a href="' . Url::log($row['logid']) . '">阅读全文&gt;&gt;</a>';
				}
			}
	...
	function getLogsForHome1($condition = '', $page = 1, $perPageNum) {
                $condition ='ORDER BY b.top DESC ,b.date DESC';
		$timezone = Option::get('timezone');
		$start_limit = !empty($page) ? ($page - 1) * $perPageNum : 0;
		$limit = $perPageNum ? "LIMIT $start_limit, $perPageNum" : '';
		$sql = "SELECT b.*,s.sortname FROM " . DB_PREFIX . "blog as b inner join ". DB_PREFIX ."sort as s on b.sortid=s.sid WHERE b.type='blog' and b.hide='n' $condition $limit";
		$res = $this->db->query($sql);
		$logs = array();
		while ($row = $this->db->fetch_array($res)) {
			$row['date'] += $timezone * 3600;
