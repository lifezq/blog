...
class Comment_Model {
	...
	function addComment($name, $content, $mail, $url, $imgcode, $blogId, $pid) 
	{
		...
		$hide = ROLE == 'visitor' ? $ischkcomment : 'n';
                ...
                global $userData;
                if($blogId!=2){ //不是留言板时再进行评论处理
                  //向博文评论用户表中加入评论用户ID
                $c_gid = $this->db->fetch_array($this->db->query("select gid,uids from ".DB_PREFIX."com_uids where gid=".$blogId));
                $c_uids=$c_gid['uids'].','.UID;
                $c_gid=$c_gid['gid'];
                
                if(!$c_gid) //如果表中该博文ID不存在 ，则插入GID
                    $this->db->query("insert into ".DB_PREFIX."com_uids (gid) values(".$blogId.")");
                $this->db->query("update ".DB_PREFIX."com_uids set uids='".$c_uids."' where gid=$blogId");
                 
                }
                $photo=(isset($userData) && $userData['photo'])?$userData['photo']:'';
		$sql = 'INSERT INTO '.DB_PREFIX."comment (date,poster,gid,comment,mail,url,hide,ip,pid,photo)
				VALUES ('$utctimestamp','$name','$blogId','$content','$mail','$url','$hide','$ipaddr','$pid','$photo')";
		$ret = $this->db->query($sql);
...
//		} else {
			...
			doAction('comment_saved', $cid);
                        $msg = '评论发表成功，请等待管理员审核';
                        if ($hide == 'n') $msg = '评论发表成功';
			emMsg($msg, Url::log($blogId));
//		}
	}
