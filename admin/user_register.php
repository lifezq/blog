...
if (isset($_GET['reSendMail'])) {
	...
	$webRoot = BLOG_URL;
	$message = <<<str
    欢迎您注册  之晴网 ， 点击以下链接激活您的帐号：
        $webRoot/?emailcheck=$emailcode
$site_desc
str;
	$subject = " 之晴网邮箱验证激活帐号，激活后即可在这里与大家分享WEB技术知识";
	if (isset($_GET['comacc']) && $_GET['comacc']) {
		$message = <<<str
    恭喜您已经完善 之晴网 帐户信息 ， 点击以下链接激活您的帐号：
...
if ($action == 'register') {
	...
	if (isset($_GET['op']) && $_GET['op'] == 'bindAccount') {
//如果是绑定帐户信息
...
//如果是完善帐户信息
		...
		*/
		$message = <<<str
    恭喜您已经完善 之晴网 帐户信息， 请点击以下链接激活您的帐号：
...
str;
		...
		$message = <<<msg
 <div style='margin:25px;line-height:30px;text-indent:2em;'>恭喜您已经完善 之晴网 帐户信息,1-3分钟内您将会收到系统给您发的激活邮件,点击进入,激活后便可登录之晴网,发文章,发碎语啦.如果长时间未收到邮箱,请点击这里再次<a href='javascript:void(0)' onClick= window.top.document.getElementById('registeriframe').src='$webRoot/admin/user_register.php?reSendMail=$email&comacc=1'; >发送激活邮件</a></div>
msg;
...
str;
	...
	$message = <<<msg
 <div style='margin:25px;line-height:30px;text-indent:2em;'>恭喜您已经注册成功,1-3分钟内您将会收到系统给您发的激活邮件,点击进入,激活后便可登录之晴网,发文章,发碎语啦.如果长时间未收到邮箱,请点击这里再次<a href='javascript:void(0)' onClick= window.top.document.getElementById('registeriframe').src='$webRoot/admin/user_register.php?reSendMail=$email'; >发送激活邮件</a></div>
msg;
