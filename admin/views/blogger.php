...
<div class="item_edit">
	...
	<?php echo $icon; ?><input type="hidden" name="photo" value="<?php echo $photo; ?>"/><br />
        <?php if(empty($password)){ if(preg_match('/qq-/',$username)){$qqsina='QQ';}else{$qqsina='新浪';} echo "<br /><span style='color:#ccc;'>默认为您当前登录{$qqsina}头像，您可以设置自己喜欢的个性头像喔</span><br/>"; } ?>
	头像<br />
    <input name="photo" type="file" /> (支持JPG、PNG格式图片)
	</li>
	<li>昵称<br /><input maxlength="50" style="width:185px;" value="<?php echo $nickname; ?>" name="name" /> <?php if(empty($password)){ echo "<br /><span style='color:#ccc;'>默认为您当前登录{$qqsina}昵称，您可以设置自己喜欢的昵称作为博客显示名称喔</span>"; } ?></li>
...
<div style="margin:30px 0px 10px;<?php if(empty($password)){ echo 'display:none;'; } ?>"><a href="javascript:displayToggle('chpwd', 2);" class="care">修改密码及登录名+</a></div>
