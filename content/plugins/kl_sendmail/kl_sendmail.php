...
function kl_sendmail_get_comment_mail()
{
	...
	if(KL_IS_SEND_MAIL == 'Y' || KL_IS_REPLY_MAIL == 'Y')
	{
		...
		{
			if(ROLE == 'visitor') sendmail(KL_MAIL_TOEMAIL, $subject, $content,'', $blogname);
		}
			...
			if($pid > 0)
			{
				...
				if(!empty($pinfo['mail']))
				{
					...
					//kl_sendmail_do(KL_MAIL_SMTP, KL_MAIL_PORT, KL_MAIL_SENDEMAIL, KL_MAIL_PASSWORD, $pinfo['mail'], $subject, $content, $blogname);
                                        sendmail($pinfo['mail'], $subject, $content);
				}
			}
		}
	}else{
...
function kl_sendmail_get_twitter_mail($r, $name, $date, $tid)
{
	...
	if(KL_IS_TWITTER_MAIL == 'Y')
	{
		...
		if(ROLE == 'visitor') sendmail(KL_MAIL_TOEMAIL, $subject, $content);
                   // kl_sendmail_do(KL_MAIL_SMTP, KL_MAIL_PORT, KL_MAIL_SENDEMAIL, KL_MAIL_PASSWORD, KL_MAIL_TOEMAIL, $subject, $content, $blogname);
...
function kl_sendmail_put_reply_mail($commentId, $reply)
{
	...
	if(KL_IS_REPLY_MAIL == 'Y')
	{
		...
		if($mail != '') sendmail($mail, $subject, $content);
